...
async def ocr_bytes_endpoint(
    ...
    if verbose:
        body["lines"] = result.lines
    return body


@router.post("/ocr/url", response_model=OcrResponse)
async def ocr_url_endpoint(payload: OcrUrlRequest, verbose: bool = False) -> dict[str, Any]:
...
async def ocr_b64_endpoint(payload: OcrB64Request, verbose: bool = False) -> dict[str, Any]:
